...
try:
    ...
    print("Création de données de démonstration à partir du contenu fourni...")
...
# ============================================
# GRAPHIQUE 1 : POURCENTAGES UNIQUEMENT
# ============================================
...
# Personnalisation
# ax1.set_title('Distribution en Pourcentage des Instances et Images par Classe', 
#               fontsize=18, fontweight='bold', pad=25)
ax1.set_ylabel('Pourcentage (%)', fontsize=16, fontweight='bold')
ax1.set_xlabel('Classes', fontsize=16, fontweight='bold')
ax1.set_xticks(x)
...
# Fonction pour placer les pourcentages uniquement
def ajouter_pourcentages(bars, valeurs, ax, decalage_base=0.5):
    """Ajoute uniquement les pourcentages"""
    positions_utilisees = []
    
    for bar, valeur_pct in zip(bars, valeurs):
        x_pos = bar.get_x() + bar.get_width()/2.
        height = bar.get_height()
        
        # Texte avec pourcentage uniquement
        texte = f"{valeur_pct:.1f}%"
        
        # Position verticale ajustable
        y_pos = height + decalage_base
        
        # Vérifier les chevauchements
        chevauchement = False
        for pos in positions_utilisees:
            if abs(pos[0] - x_pos) < 0.3 and abs(pos[1] - y_pos) < 1.5:
                chevauchement = True
                break
        
        # Ajuster la position si chevauchement
        if chevauchement:
            y_pos = height + decalage_base + 0.8
...
# Ajouter les pourcentages uniquement
ajouter_pourcentages(bars1, pourcentage_objets, ax1, decalage_base=0.3)
ajouter_pourcentages(bars2, pourcentage_img, ax1, decalage_base=0.3)
